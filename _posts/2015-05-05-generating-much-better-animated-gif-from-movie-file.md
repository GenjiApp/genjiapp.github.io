...
　今回の手法はImageMagickは用いず、ffmpegだけを用いる。ffmpegはv2.6以上必須。動画はffmpegが扱えるならなんでも良いと思う。
...
　入力動画、[ImageMagickを用いた手法（前回の手法）]({% post_url 2014-06-04-generating-better-animated-gif-from-mov-recorded-by-quicktime-player %})で生成したもの、パレット画像を使用せずに生成したもの、パレット画像を使用して（今回の手法で）生成したものを以下に示す。
...
　前回の手法で生成したアニメーションGIFはテキストエディタのウィンドウの背景が薄青く変色してしまっているんのが解る。これはまだましな方で、動画によってはもっと大きく色が崩れてしまうことがある（前フレームの残像が残ったり）。パレット画像不使用の場合はドロップシャドウやメニューなど、半透明がからむ部分で編みかけのようになってしまっている。それらに比べて今回の手法で生成したものは、そういう画像の乱れが見られない。
　また、処理にかかった時間もパレット画像不使用に比べれば遅いものの、前回の手法の方よりは大幅に速い。出力ファイルサイズも前回の手法よりは小さく、パレット画像不使用よりは大きいという結果になった。
...
　日本人的には、OS標準の日本語IMやATOK、Google日本語入力等の日本語インプットメソッドを通じて絵文字の入力はある程度容易に、いつも通りの操作で行えるが、ふだんインプットメソッドを意識しないで文字入力している層（英語圏など）はどうしているんだろうか。わざわざ文字ビューアを起動して、検索して、選択して、とやっているのか——そういうところから着想を得て現在鋭意開発中。
